...
const MyList = () => {
    ...
    useEffect(() => {
        const fetchMovies = async () => {
            const snapshot = await firestore().collection('USERS').doc(firebase.auth().currentUser.email).collection('myList').get();
            ...
            />
            {
                !loading && movies?.length === 0 && (
                    <View style={styles.warningWrapper}>
                        <Text style={styles.warning}>There are no movies in your list.</Text>
                    ...
                    {movies?.map((movie, index) => (
                        <TouchableOpacity key={index} activeOpacity={0.5} onPress={() => navigation.navigate("VideoPlayer", { id: movie.movieID })}>
                            <View style={styles.movieCard}>
                                <Image style={styles.moviePoster} resizeMode='cover' source={{ uri: movie.banner }} />
                            </View>
                        </TouchableOpacity>
                    ))}
                </View>
